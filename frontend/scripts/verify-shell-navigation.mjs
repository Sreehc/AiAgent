...
function main() {
  ...
  assertContains("src/components/shell/Topbar.tsx", /<IconButton[\s\S]*className="topbar__theme-toggle"/, "Theme toggle should expose a dedicated topbar class for alignment");
  ...
  assertContains("src/styles/layout.css", /\.topbar__theme-toggle \{\s*min-height:\s*var\(--control-height-sm\);/, "Theme toggle should align to the shared topbar control height");
  assertContains("src/styles/layout.css", /\.command-trigger \{\s*min-height:\s*var\(--control-height-sm\);/, "Command trigger should keep the shared topbar control height");
