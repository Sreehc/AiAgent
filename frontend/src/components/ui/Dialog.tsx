...
export function Dialog({ isOpen, title, description, children, footer, onClose, className }: DialogProps) {
  return (
    <RadixDialog.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <RadixDialog.Portal>
        ...
        <RadixDialog.Content className={cn("fixed left-1/2 top-1/2 z-[50] flex w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-lg border border-border bg-card p-6 shadow-lg focus:outline-none", className)}>
