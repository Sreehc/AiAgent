...
type ApiConfigFormProps = {
  ...
  configured: boolean;
  saving: boolean;
  testState: AccountApiConfigTestResult["modelType"] | null;
  lastTestResult: AccountApiConfigTestResult | null;
  message: string | null;
  error: string | null;
...
export function ApiConfigForm({ config, apiKeyMasked, configured, saving, testState, lastTestResult, message, error, onChange, onSubmit, onTest }: ApiConfigFormProps) {
  return (
    <Panel className="account-api-card" title="个人 API 配置" eyebrow="AI service" description="配置个人研究任务使用的 OpenAI-compatible 服务。">
      <form className="form-grid" onSubmit={onSubmit}>
        {error ? <Alert tone="error">{error}</Alert> : null}
        {message ? <Alert tone="success">{message}</Alert> : null}
        {lastTestResult ? <Alert tone={lastTestResult.status === "SUCCESS" ? "success" : "error"} title={`${lastTestResult.modelType} 测试结果`}>{lastTestResult.message}</Alert> : null}
        ...
        <div className="cluster">
          <Button type="button" variant="secondary" loading={testState === "CHAT"} disabled={saving || !configured} onClick={() => onTest("CHAT")}>测试 Chat</Button>
          <Button type="button" variant="secondary" loading={testState === "EMBEDDING"} disabled={saving || !configured} onClick={() => onTest("EMBEDDING")}>测试 Embedding</Button>
          <Button type="button" variant="secondary" loading={testState === "IMAGE"} disabled={saving || !configured} onClick={() => onTest("IMAGE")}>测试 Image</Button>
