...
import { AccountApiConfig } from "../../services/accountApi";
...
type ApiConfigFormProps = {
  ...
  message: string | null;
  error: string | null;
  onChange: (config: AccountApiConfig) => void;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
};

export function ApiConfigForm({ config, saving, message, error, onChange, onSubmit }: ApiConfigFormProps) {
  return (
    <Panel title="个人 API 配置" eyebrow="AI service" description="配置个人研究任务使用的 OpenAI-compatible 服务。">
      <form className="form-grid" onSubmit={onSubmit}>
        ...
        <Field label="API Key" description="密钥将提交到服务器保存。"><Input type="password" value={config.apiKey} onChange={(event) => onChange({ ...config, apiKey: event.target.value })} placeholder="sk-..." required /></Field>
