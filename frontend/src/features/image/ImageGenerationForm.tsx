  ...
  return (
    <Panel title="生成参数" eyebrow="Controls">
      <form className="form-grid" onSubmit={onSubmit}>
        ...
        {form.mode === "EDITS" ? <Field label="参考图" description={referenceFile ? `已选择：${referenceFile.name}` : "上传一张需要编辑的参考图片。"}><Input type="file" accept="image/*" onChange={(event) => onReferenceFileChange(event.target.files?.[0] ?? null)} required /></Field> : null}
