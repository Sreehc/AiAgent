...
export function KnowledgeBaseList({ items, selectedId, loading, submitting, form, onFormChange, onCreate, onSelect, onRefresh }: KnowledgeBaseListProps) {
  return (
    <aside className="stack">
      ...
      <Panel title="知识库列表" eyebrow="Library" action={<Button type="button" variant="ghost" size="sm" onClick={onRefresh}>刷新</Button>}>
        <div className="list">
          {loading ? <Skeleton lines={4} compact /> : null}
          {!loading && items.map((item) => (
            <button key={item.kbId} type="button" className={`list-item ${selectedId === item.kbId ? "list-item--active" : ""}`} onClick={() => onSelect(item)}>
              <div className="split"><strong>{item.name}</strong><StatusPill status={item.status} /></div>
              <span>{item.description || "暂无描述"}</span><small>{item.documentCount} 个文档 · {formatDateTime(item.updatedAt ?? item.createdAt)}</small>
            </button>
          ))}
