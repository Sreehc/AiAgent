...
request_json POST "${BASE_URL}/api/v1/auth/register-by-invite" "{\"inviteToken\":\"${INVITE_TOKEN}\",\"username\":\"${TEMP_USER}\",\"displayName\":\"Smoke User\",\"password\":\"Passw0rd!\",\"confirmPassword\":\"Passw0rd!\"}" "" "$TMP_DIR/register.json" "$TMP_DIR/register.status"
...
assert_contains '"code":"AUTH_FORBIDDEN"' "$TMP_DIR/forbidden.json"
